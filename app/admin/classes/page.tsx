...
export default function ManageClasses() {
  ...
  const fetchClasses = async () => {
    try {
      const { getAssignedSchoolIds, isSuperAdmin } = await import("@/lib/auth");
      const res = await axios.get<Class[]>(`${API_BASE}/ClassDivision/getclasses`)
      const data = res.data;
      const filtered = isSuperAdmin() ? data : data.filter((c: Class) =>
        getAssignedSchoolIds().includes(c.schoolId));
      setClasses(filtered)

      // fetch divisions after classes ready
      const divRes = await axios.get<Division[]>(`${API_BASE}/ClassDivision/getdivisions`)
      const divData = divRes.data;
      const classIds = filtered.map((c: Class) => c.classId);
      const filteredDivs = isSuperAdmin() ? divData : divData.filter((d: Division) => classIds.includes(d.classId));
  ...
  try {
    ...
    const data = res.data;
    // Filter divisions to only show ones belonging to filtered classes
    const classIds = classes.map(c => c.classId);
    const filtered = isSuperAdmin() ? data : data.filter((d: Division) => classIds.includes(d.classId));
