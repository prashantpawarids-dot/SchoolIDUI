...
import { buildDesignerPrintHtml } from "../../admin/print/lib/card-renderer";
...
export default function PrintIDCards() {
  ...
  const loadSchools = async () => {
    ...
    setSchools((data || []).filter((s: any) => s.schoolId === schoolId));
  };
  ...
  const loadStudents = async () => {
  const schoolId = Number(localStorage.getItem("schoolId"));
  const data = await fetch(`${BASE_URL}/Student/getalwithstatus?schoolId=${schoolId}`)
    .then(r => r.json());
  ...
  const loadHistory = async () => {
    try {
      const schoolId = Number(localStorage.getItem("schoolId"));
      const data = await fetch(`${BASE_URL}/Print/jobs?schoolId=${schoolId}`).then(r => r.json());
      setJobHistory(data || []);
    } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    ...
    setSelectedIds([]);
  }, [selYear, selClass, selDiv, students]);
...
<body>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── ✅ PageHeader shows school name ── */}
      <PageHeader
        title="Print ID Cards"
        description={schools.length > 0 ? schools[0].schoolName : "Professional card printing"}
      />

      {/* ── FILTERS ── */}
      <Card>
        {/* ── ✅ School dropdown removed ── */}
