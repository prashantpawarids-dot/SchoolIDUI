...
const FIELD_POOL: Omit<FieldPosition, "x" | "y">[] = [
  ...
  { key: "bloodGroup",    label: "Blood Group",       fontSize: 9,  fontColor: "#cc0000", bold: true,  italic: false, visible: true,  isImage: false, width: 25, height: 5,  align: "left"   },
...
const DEFAULT_FIELDS: FieldPosition[] = [
  ...
  { key: "bloodGroup",    label: "Blood Group",    x: 65, y: 75, fontSize: 9,  fontColor: "#cc0000", bold: true,  italic: false, visible: true,  isImage: false, width: 25, height: 5,  align: "left"   },
...
export default function CardDesignerPage() {
 const { schools } = useSchools();
  ...
  // Auto-select own school for school role
  // Auto-select own school for school role
  useEffect(() => {
    ...
    if (schools && schools.length > 0) {
      const ownSchool = schools.find(s => s.schoolId === schoolId);
      if (ownSchool && !selSchool) { setSelSchool(ownSchool as any); return; }
    }
    // Fallback — fetch directly
    fetch(`${BASE_URL}/School/list`)
      .then(r => r.json())
      .then(data => {
        const ownSchool = (data || []).find((s: any) => s.schoolId === schoolId);
        if (ownSchool && !selSchool) setSelSchool(ownSchool);
      })
      .catch(() => {});
  }, [schools]);
    ...
    <div className="flex flex-col lg:flex-row h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="w-full lg:w-64 bg-white border-b lg:border-b-0 lg:border-r flex flex-col shrink-0 order-2 lg:order-1 max-h-[40vh] lg:max-h-none">
        <div className="p-4 border-b bg-blue-700 text-white">
          <a href="/school/print" className="flex items-center gap-1 text-blue-200 hover:text-white text-xs mb-2 transition-colors">
            ← Back to Print
          </a>
        ...
        <div className="p-3 space-y-3 flex-1 overflow-y-auto">
          ...
          <div>
            ...
            <select
              ...
              {schools.filter(s => s.schoolId === Number(localStorage.getItem("schoolId"))).map(s => (
                  ...
                  return (
                    <div key={f.key} style={baseStyle} onMouseDown={e => handleMouseDown(e, f.key)}>
                      {src
                        ...
                        : <div className="w-full h-full bg-white/20 rounded flex items-center justify-center border border-dashed border-white/40">
                            <span className="text-white text-[9px] opacity-70">{f.label}</span>
                          </div>
                      }
                      {isSelected && (
