...
useEffect(() => {
  ...
  const filteredStudents = students.filter((s) => {
    if (selectedSchool !== "all" && s.schoolId?.toString() !== selectedSchool)
      return false;
    if (selectedYear !== "all" && s.academicYearId?.toString() !== selectedYear)
      return false;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Export students using filters and preview before download"
      />
        ...
        {exportOptions.map((option) => {
          ...
          return (
            <Card key={option.type} className="shadow-lg border-0">
              <CardHeader>
                <div
                  className={`w-14 h-14 rounded-xl ${option.iconBg} flex items-center justify-center mb-3`}>
